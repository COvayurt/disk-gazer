...
namespace DiskGazer.ViewModels
{
	public class MainWindowViewModel : NotificationObject
	{
		...
		private async Task SaveScreenshotLog()
		{
			...
			catch (Exception ex)
			{
				MessageBox.Show($"Failed to save screenshot and log. {ex.Message}",
								"Errpr", MessageBoxButton.OK, MessageBoxImage.Error);
				return;
			}
		...
		private System.Drawing.Bitmap GetScreenshot()
		{
			try
			{
				var screenshot = new System.Drawing.Bitmap((int)300, (int)300);
				using var g = System.Drawing.Graphics.FromImage(screenshot);
				g.CopyFromScreen(new System.Drawing.Point((int)100, (int)100), System.Drawing.Point.Empty, screenshot.Size);
				return screenshot;
			}
			catch (Exception ex)
			{
		...
		private string ComposeLog()
		{
			...
			{
				string.Format("[{0}]", "Disk Gazer v1.2"),
